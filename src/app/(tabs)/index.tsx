...
export default function HomeScreen() {
  ...
  // 1. DATA REFRESH: Runs every time you navigate to this tab
  useFocusEffect(
    useCallback(() => {
      fetchStats();
  ...
  // 2. ANIMATION: Runs only once when the app starts
  useEffect(() => {
    startBreathing();
  }, []);
  ...
  const refreshRecommendation = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        // If there are no pending items, clear the recommendation
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text style={styles.name}>Shreyas</Text>
